...
int perform(char*cmd)
{
	...
	{		
		int i=1;
		if((cinp[i][0]!='"')&&cinp[i][0]=='$')
		{
			char* varu=(char*)malloc(sizeof(char)*(strlen(cmd)));
			if(cinp[1][1]!='\0');for(int j=1;cinp[1][j]!='\0';j++)
			{varu[j-1]=cinp[1][j];}
			char*result=getenv(varu);if(result!=NULL)printf("%s ",result);
			else
	...
	{
		if(cind>1){
			if(cind>2){printf("ERROR: Too many arguments to Pinfo \n");perror("pinf");return -1;}
			else
